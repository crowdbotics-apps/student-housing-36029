...
export default class RNVideoPlayer extends Component {
  static defaultProps = {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      volumeFillWidth: 0,
      seekerFillWidth: 0,
      showControls: this.props.showOnStart || true,
      volumePosition: 0,
      seekerPosition: 0,
      volumeOffset: 0,
      seekerOffset: 0,
    ...
    this.opts = {
      ...
      playInBackground: this.props.playInBackground  || true,
  ...
  _onError(err) {
    ...
    this.setState(state);
    console.log('vido player _onError: ', err);
  }
  ...
  renderVolume() {
    ...
    return (
        <Icon name={vol == 0 ? 'volume-off': vol == 1 ? 'volume-up' : 'volume-down'} type='material' color={'#FFF'} size={20} onPress={() => this.setState({ volume: vol==0 ? 1 : 0 })}/>
    );
  }
  ...
  renderFullscreen = () => {
    ...
    let icon =
      this.state.isFullscreen === true
        ?  <Icon name={'fullscreen-exit'} type='material-community' color={'#FFF'} size={20} onPress={onPress} />
        :  <Icon name={'fullscreen'} type='material-community' color={'#FFF'} size={20} onPress={onPress}/>;
    return icon
  }
        ...
        <Animated.View
          ...
          <ImageBackground
            ...
            imageStyle={[styles.controls.vignette]}>
              <View style={{ width: '90%', height: 50, alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', }}>
                {Title}
                <View style={{ width: 50, height: 30, alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', }}>
                  {volumeControl}
                  {fullscreenControl}
                </View>
  ...
  renderPlayPause() {
    let icon =
      this.state.paused === true
        ...
        :  <Icon name={'pause'} type='ionicon' color={'#FFF'} size={20} />;
  ...
  renderTimer() {
    return this.renderControl(
      <LatoText style={styles.controls.timerText}>{this.calculateTime()}</LatoText>,
